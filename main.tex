...
\begin{figure}
    ...
    \caption{Comparison of direct correlation function of water and hard sphere fluid:\\
...
\begin{figure}
    ...
    \caption{Van-der-Waals functions for the coarse-grained densities:\\
    Those functions of eq.~\ref{eq:F_VdW} describe  the (near) coexistence of liquid and vapor for the coarse-grained densities. The blue dashed line and black line correspond to two different radius for the hard-spheres of the reference fluid:  $R_0 = 1.25$~\AA~ and $R_0 = 1.4$~\AA~, respectively.
        \label{fig:fn}
        }
\end{figure}
...
\begin{figure}
    ...
    \caption{
        ...
        Water stucture around a neopentane molecule:\\At the top, the neopentane molecule and the associated three-dimensional water density obtained by functional minimization. At the bottom, the radial distribution function between the neopentane center of mass and the oxygen of water as calculated by MDFT with hard-sphere bridge and Van-der-Waals correction with atomic partial charges (black line) and without (blue circle). The reference radial distribution function as calculated by MD is shown in red.
   ...
   \includegraphics[width=8.5cm]{cyclohexane_g_and_dens_with_legend.pdf}
    \caption{
        \label{fig:gr_cyclohexane}
                Water structure around a cyclohexane molecule:\\At the top, the cyclohexane molecule and the associated three-dimensional water density obtained by functional minimization. At the bottom, the radial distribution function between the cyclohexane center of mass and the oxygen of water as calculated by MDFT with hard-sphere bridge and Van-der-Waals correction with atomic partial charges (black line) and without (blue circle). The reference radial distribution function as calculated by MD is shown in red.
%        Comparison of the solute center-of-mass/water oxygen radial distribution function computed by molecular dynamics (red line) and MDFT
...
\begin{figure}
    ...
    \caption{
        ...
        Comparison of solvation free-energies values for a set of alcanes:\\
...
\begin{figure}
    ...
    \caption{
        ...
        Solvation free energy of soft-repulsive spheres (see text) for different radius $R$:\\
        The results are obtained by MDFT minimization using simply the hard-sphere bridge term of eqs~\ref{eq:Fn_exc}-\ref{eq:Fn_bridge} (blue squares) or the hard-sphere bridge term plus long range Van-der-Waals correction of eq.~\ref{eq:Fn_exc_2} (black circles). The reference MD results of Dzubiella and Hansen\cite{dzubiella04} are shown in red. The arrow
